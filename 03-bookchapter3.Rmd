...
In context, Bradley's were in response to his former British colleague and fellow hero of World War Two, retired British Field Marshall Viscount Bernard Montgomery, who days earlier had publicly criticized the leadership of US President Dwight Eisenhower, who also was a retired five star general and previous boss of both Montgomery and Bradley in Europe during the second World War [@times_bradley_1959]. By admonishing Montgomery, Bradley seems to convey the idea that it simply is not wise for a popular retired military figure to weigh in on contemporary political issues. Though he does not explicitly invoke the existence of any central principles of civil-military relations in defending his remarks, it is nevertheless clear that Bradley is concerned about the potential for retired officers --- especially those with high rank --- to influence politics inappropriately by speaking out. Such acts to Bradley might constitute the _abuse_ of one's military rank. 
...
This main and second section of the chapter analyzes and interprets the data. First, this section presents the data and offers summary statistics of the data. This will help the reader become better oriented to the data. Second, this section then dives deeper into the data by conducting a thorough statistical analysis of the data. In this part of the analysis in particular, we will examine the general topics that retired military officers have addressed in what the opinion commentary.  We will then separate observations within the data between instances that constitute a violation of the central principles of civil-military relations from those that do not. After this important investigation, this section turns to interpret the data. We will see that that the data will help us answer both H1 and H2, but to very different degrees.  
...
topic_graph <- topic_graph_df %>% ggplot(aes(x=PubYr, y=Proportion, linetype=statistic, color=statistic)) + 
  geom_smooth(se=FALSE, linewidth=.25) + 
  labs(x="Year", y="Proportion of Commentary") + 
  scale_linetype_manual(values= c("solid", "solid", "solid", "dashed", "dashed", "dashed", "dotdash", "dotdash"), name="Topic",
  ...
  scale_color_manual(values =c("black", "gray","white", "black", "gray", "white", "black", "gray"), name="Topic",
                         breaks=c("bud_prop", "civ_mil_prop", "dom_pol_prop", "fp_prop", "soc_pol_prop", "spt_prop", "svc_cul_prop", "war_prop"), labels=c("Budget/Weapons/Troops", "Civil-Military Relations","Domestic Policy", "Foreign Policy","Social Policy", "Support", "Service Culture", "Warfighting"))+
  theme(legend.position="bottom") + 
  geom_vline(xintercept = 1981, size=.2, colour="black")+
  geom_vline(xintercept=1989, size=.2, colour="black") + 
